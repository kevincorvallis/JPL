...
def openfile(inputFilename):
    with open(inputFilename) as fp:
        inputdata = json.load(fp,object_hook=_byteify)
    return inputdata
def main():
    # ------------------------------------- Initialization and Input Parameters -------------------------------
    # inputfile = 'submasterDurations_sols1000to2169_preUpdate.json'
    inputfile = '0_2400Soles.json'
    doPlot = True
    doPercentiles = True
    # ------------------------------------- Open json file --------------------------------------------
    ...
    if doPlot == True:
        ...
        # plt.subplot(amountUnusedTimeArr)
        print(amountUnusedTimeArr)
...
##############################################################################################################
##############################################################################################################
def makeLinePlot(figNum, title, xLabel, yLabel, dataArr , binOverrides = None):
    """
    Inputs: 
